...
// Récupération des infos et messages d'erreur
...
<form method="post" action="../tools/business/check_info_ad.php">
	<table>
		...
		<tr>
			...
			<td><input type="text" name="title" value="<?php echo $form_data[_TITLE];?>"/><?php if ($rank == _TITLE) echo $msg; ?></td>
		</tr>
		...
		<tr>
			...
			<td><select name="category">
				...
				<option value=4 <?php if ($form_data[_CATEGORY] == 4) echo 'selected="selected"'?>><?php echo _CATEGORIE_CARTONS?></option>
				<option value=5 <?php if ($form_data[_CATEGORY] == 5) echo 'selected="selected"'?>><?php echo _CATEGORIE_PALETTES?></option>
				<option value=6 <?php if ($form_data[_CATEGORY] == 6) echo 'selected="selected"'?>><?php echo _CATEGORIE_VEHICULES?></option>
				<option value=7 <?php if ($form_data[_CATEGORY] == 7) echo 'selected="selected"'?>><?php echo _CATEGORIE_DEUX_ROUES?></option>
		...
		<tr>
			...
			<td><input type="text" name="departure_city" value="<?php echo $form_data[_DEPARTURE_CITY]?>"/><?php if ($rank == _DEPARTURE_CITY) echo $msg; ?></td>
		</tr>
		<tr>
			<td><?php echo _AD_DESTINATION_CI . ' : '?></td>
			<td><input type="text" name="destination_city" value="<?php echo $form_data[_DESTINATION_CITY]?>"><?php if ($rank == _DESTINATION_CITY) echo $msg; ?></td>
		</tr>
		<!-- Dates -->
		<tr>
			<td><?php echo _AD_DATE_BEG . ' : '?></td>
			<td><input type="text" name="date_beginning" readonly="readonly" value="<?php echo $form_data[_DATE_BEGINNING]?>"/>
			<input type="button" value="<?php echo _SELECTION?>" onclick="displayDatePicker('date_beginning', false, 'dmy', '.');"/>
			<?php if ($rank == _DATE_BEGINNING) echo $msg; ?></td>
		</tr>
		<tr>
			<td><?php echo _AD_DATE_END . ' : '?></td>
			<td><input type="text" name="date_end" readonly="readonly" value="<?php echo $form_data[_DATE_END]?>" />
			<input type="button" value="<?php echo _SELECTION?>" onclick="displayDatePicker('date_end', false, 'dmy', '.');"/>
			<?php if ($rank == _DATE_END) echo $msg; ?></td>
		</tr>
		<!-- Poids total, nombre d'objects, volume total -->
		<tr>
			<td><?php echo _AD_TOTAL_WEIGHT . ' [kg] : '?></td>
			<td><input type="text" name="total_weight" value="<?php echo $form_data[_TOTAL_WEIGHT]?>" />
			<?php if ($rank == _TOTAL_WEIGHT) echo $msg;?></td>
		</tr>
		<tr>
			<td><?php echo _AD_OBJECT_NUM . ' : '?></td>
			<td><input type="text" name="objects_num" value="<?php echo $form_data[_OBJECTS_NUM];?>" />
			<?php if ($rank == _OBJECTS_NUM) echo $msg;?></td>
		</tr>
		<tr>
			<td><?php echo _AD_TOTAL_VOLUME . ' [L] : '?></td>
			<td><input type="text" name="total_volume" value="<?php echo $form_data[_TOTAL_VOLUME]; ?>"/>
